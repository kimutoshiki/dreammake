...
### ⭐ デモフロー(ログイン不要、児童用と教員用は別ページ)

ログインシステムはありません。`/` から **こどもの ページ** / **せんせいの ページ**
を 選ぶと、それぞれの 独立した 画面へ進みます。児童ページは 右上の 切替で
みさき / たけし / ゆい の 3 人を 入れ替えできます(Cookie 保存、30 日)。

1. `http://localhost:3000/` を開く
2. **こどもの ページ** へ入る
   - 右上のセレクタで「みさき」「たけし」「ゆい」を切替
   - 「わたしたちの町の昔と今」単元を開く
   - 🔍 **「声が聞こえていないのはだれ?」**(本アプリの中核機能)
   - ✍️ **「ふりかえりを 書く」** — 立ち止まりの言葉が 書きながら ハイライト
   - 🗺️ **「立場マップ」** で 自分の立場と クラスの分布を見る
   - 🎮 **「立場クイズ・少数派さがし」**(新)— 友だちの書いた「なぜ?」から 立場を当てる
   - 📊 **「この単元の まとめ」**(新)— 立場分布・立ち止まり語・声の仮説 を 一目で
   - 📋 事前/事後 アンケート(3 軸)
   - 🤖 「町はかせ」ボットと 対話(ナレッジ制約 + 出典自動付与 + 入力モデレーション)
3. **せんせいの ページ** へ入る(トップから)
   - ダッシュボード / 単元編集 / 時数設計 / 立場追加 / アンケートテンプレ生成
   - 📝 児童のふりかえり(ハイライト付)/ 🔍 声の仮説 / 🗺️ 立場分布 / 📋 アンケート結果 を 閲覧
