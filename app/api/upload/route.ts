...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      // Vercel の 読み取り専用 fs に 書こうとして 落ちた 場合の 案内
      const hint = /EACCES|EROFS|read-only/i.test(msg)
        ? 'サーバの ストレージ が 読み取り専用です。先生に Vercel Blob を 有効に してもらってください。'
        : 'ほぞんに しっぱい しました。';
      return NextResponse.json(
        { error: hint, detail: msg.slice(0, 200) },
        { status: 500 },
      );
    }
    ...
    const msg = (err as Error)?.message ?? String(err);
    return NextResponse.json(
      { error: 'サーバが エラーで 返した よ', detail: msg.slice(0, 200) },
      { status: 500 },
    );
  }
}
